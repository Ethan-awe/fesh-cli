...
void encrypt_bmp(const char* input_bmp, const char* output_bmp, u8* key, int mode) {
    FILE *in_fp = NULL, *out_fp = NULL;
    u8 header[HEADER_SIZE];

    fopen_s(&in_fp, input_bmp, "rb");
    if (!in_fp) {
        printf("Failed to open input BMP: %s\n", input_bmp);
        return;
    }
    ...
    fread(data, 1, data_size, in_fp);
    fclose(in_fp);

    int padded_size = ((data_size + 15) / 16) * 16;
    u8* padded = (u8*)calloc(padded_size, 1);
    memcpy(padded, data, data_size);
    free(data);

    u8* encrypted = (u8*)malloc(padded_size);
    int out_len = 0;

    if (mode == 1) {
        Crypt_Enc_Block_CBC(padded, padded_size, encrypted, &out_len, key, 16);
        printf("CBC encryption done.\n");
    } else {
        Crypt_Enc_Block(padded, padded_size, encrypted, &out_len, key, 16);
        printf("ECB encryption done.\n");
    }
    ...
    fwrite(encrypted, 1, data_size, out_fp);
    fclose(out_fp);
...
int main(int argc, char* argv[]) {
    if (argc != 3) {
        printf("Usage: pic_enc <input_image.bmp> <ECB|CBC>\n");
        return -1;
    }
    ...
    sprintf(output_file, "fig/c_%s.bmp", mode_str);

    encrypt_bmp(input_file, output_file, key, mode);
    return 0;
}
